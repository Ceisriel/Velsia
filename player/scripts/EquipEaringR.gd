...
onready var attachment = get_parent().get_node("Mesh/Armature/Skeleton/EaringR")
onready var camera = $"../Camroot/h/v/Camera"
onready var detector = $"../Detector"
onready var earingL = get_parent().get_node("EquipEaringL")
...
func drop():
	if currentInstance != null and Input.is_action_just_pressed("drop"):
		...
		currentInstance.global_transform.origin = Vector3(drop_position.x + 0.15, player.global_transform.origin.y + 0.2, drop_position.z)
...
func _on_Detector_body_entered(body):
	if currentInstance == null:
		if earingL.currentInstance != null:
